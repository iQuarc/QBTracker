...
namespace QBTracker.ViewModels
{
   public class ExportViewModel : ValidatableModel
   {
      ...
      public ExportViewModel(MainWindowViewModel mainVm)
      {
         ...
         CommandManager.RequerySuggested += (sender, args) => { this.NotifyOfPropertyChange(nameof(ProjectsTitle)); };
      }
      ...
      public void Activated()
      {
         ...
         this.NotifyOfPropertyChange(nameof(ProjectsTitle));
      }

      public string ProjectsTitle
      {
         get => $"Projects {this.ExportProjects.Count(x => x.IsSelected)}/{this.ExportProjects.Count}";
      }
      ...
      private IEnumerable<IGrouping<string, ExportRecord>> GroupByWorkSheet(IEnumerable<ExportRecord> timeRecords)
      {
